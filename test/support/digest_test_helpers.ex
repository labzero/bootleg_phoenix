defmodule BootlegPhoenix.DigestTestHelpers do
  ...
  defmacro digest_test(fixture) do
    ...
    quote do
      ...
      test "phoenix_digest generates the digest during compile (#{unquote(version_string)})", %{app_location: location, hosts: hosts} do
        shell_env = [
        ...
        Enum.each(["deps.get", "bootleg.build", "bootleg.deploy", "bootleg.start"], fn cmd ->
          ...
          if cmd == "bootleg.build" do
            assert String.match?(out, ~r/Phoenix asset digest generated/), "Digest genration failed"
          end
        end)
      end
    end
